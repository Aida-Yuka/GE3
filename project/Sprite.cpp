...
void Sprite::Initialize(SpriteBase* spriteBase)
{
	...
	///=====マテリアルの作成=====///
	//マテリアルリソースを作る
	materialBuffer = this->spriteBase->GetDirectXBase()->CreateBufferResource(sizeof(Vector4)).Get();
	...
	///=====座標変換行列=====///
	//座標変換行列リソースを作る
	ConstantBuffer = this->spriteBase->GetDirectXBase()->CreateBufferResource(sizeof(Matrix4x4));
	//materialResourceに書き込むためのアドレスを取得して、transformationMatrixDataにデータを書き込む
	ConstantBuffer->Map(0, nullptr, reinterpret_cast<void**>(&transformationMatrixData));
...
void Sprite::Draw()
{
	...
	//dxBase->GetCommandList()->SetGraphicsRootConstantBufferView(1, ConstantBuffer->GetGPUVirtualAddress());
	////マテリアルCBufferの場所を設定
	//dxBase->GetCommandList()->SetGraphicsRootConstantBufferView(0, materialBuffer->GetGPUVirtualAddress());

	////dxBase->GetCommandList()->SetGraphicsRootDescriptorTable(2, textureSrvHandleCGU);
	////描画
